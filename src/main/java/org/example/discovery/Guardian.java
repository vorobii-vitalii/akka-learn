...
public class Guardian {
	...
	public static Behavior<Void> create() {
		return Behaviors.setup((ActorContext<Receptionist.Listing> context) -> {
			context.getSystem().receptionist().tell(Receptionist.subscribe(
					ServiceKeys.PING_SERVICE_KEY,
	...
	private Behavior<Receptionist.Listing> onListing(Receptionist.Listing msg) {
		msg.getServiceInstances(ServiceKeys.PING_SERVICE_KEY)
				.forEach(pingService -> context.spawnAnonymous(Pinger.create(pingService)));
		return Behaviors.same();
	}

	public static void main(String[] args) {
		ActorSystem.apply(Guardian.create(), "discovery");
	}
